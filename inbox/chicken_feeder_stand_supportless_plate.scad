...
$fn = 64;
part = "plate"; // "stand", "leg", "legs4", "plate", "assembly"
...
module plate() {
    // Everything on one 256 x 256 plate, still as separate detachable parts.
    // Stand stays flat at the front; four legs are arranged behind it.
    stand();

    leg_gap = 12;
    leg_start_y = stand_outer_d + 16;
    leg_start_x = 72;
    for (ix = [0:1], iy = [0:1])
        translate([leg_start_x + ix*(foot_size + leg_gap),
                   leg_start_y + iy*(foot_size + leg_gap),
                   0])
            leg();
}
