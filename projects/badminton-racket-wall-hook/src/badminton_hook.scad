...
// Shaft pushes in from the selected side through a narrow entry gap,
// snaps into a circular pocket. Lips + gravity hold it in place.
//
//     Top view, entry_side = "right":
//     ┌──────────────────────────────┐
//     │    (O)──────┤  (O)──────┤   │  ← entry on right face (X+)
//     └──────────────────────────────┘  ← wall plate
//
//     Top view, entry_side = "left":
//     ┌──────────────────────────────┐
//     │    ├──────(O)  ├──────(O)   │  ← entry on left face (X-)
//     └──────────────────────────────┘
...
/* ── Keyhole slot void ────────────────────────────────────────────────────
   ...
   Entry channel runs from pocket center to the selected side face.
   ─────────────────────────────────────────────────────────────────────── */
module slot_void(y_center) {
    ...
    // Entry channel toward selected side
    if (entry_side == "right") {
        // Entry from right face (X+): channel from pocket center → X=arm_w
        translate([arm_w / 2, y_center - entry_gap / 2, -0.1])
            cube([arm_w / 2 + 0.1, entry_gap, arm_h + 0.2]);
    } else {
        // Entry from left face (X-): channel from X=0 → pocket center
        translate([-0.1, y_center - entry_gap / 2, -0.1])
            cube([arm_w / 2 + 0.1, entry_gap, arm_h + 0.2]);
    }
}
...
module wall_plate() {
    difference() {
        ...
        // Screw holes above arm (arm occupies z=0..arm_h) — always accessible
        for (x = [plate_w * 0.25, plate_w * 0.75])
            for (z = [arm_h + 5, plate_h - 5])
                translate([x, -0.1, z])
                    rotate([-90, 0, 0])
