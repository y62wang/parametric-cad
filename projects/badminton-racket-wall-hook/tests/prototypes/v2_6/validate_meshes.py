#!/usr/bin/env python3
"""Validate v2.6 meshes and protected v2.2-v2.5 assets with the stdlib."""
...
EXPECTED_COMPONENTS = {
    "v2_6_connected_body_start_left_screwless.3mf": 1,
    "v2_6_connected_body_start_right_screwless.3mf": 1,
    "v2_6_skadis_back_two_hook_slide_lock.3mf": 1,
    "v2_6_anti_lift_clip.3mf": 1,
    "v2_6_skadis_two_hook_fit_coupon.3mf": 1,
    "v2_6_dovetail_tolerance_coupon_020_030_040.3mf": 4,
}
...
PROTECTED_MANIFEST = Path(__file__).with_name(
    "protected_v2_2_v2_3_v2_4_v2_5.sha256"
)
EXPECTED_PROTECTED_ASSET_COUNT = 83
...
def verify_source_is_skadis_only() -> list[str]:
    if not SOURCE.is_file():
        return [f"missing v2.6 source: {SOURCE}"]
    ...
    for marker in FORBIDDEN_SOURCE_MARKERS:
        if marker in source_text:
            failures.append(
                f"forbidden legacy marker in v2.6 source: {marker}"
            )
    if not failures:
        print(
            "v2.6 source: SKADIS-only; no legacy board or "
            "fastener geometry markers"
        )
    return failures
...
def verify_inventory(directory: Path) -> list[str]:
    ...
    for unexpected in sorted(actual - expected):
        failures.append(
            f"unexpected v2.6 3MF in package: {unexpected}"
        )
    return failures
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        nargs="?",
        type=Path,
        default=Path("files/prototypes/v2.6"),
    )
    arguments = parser.parse_args()

    failures = []
    failures.extend(verify_source_is_skadis_only())
    ...
    print(
        "\nV2_6_SKADIS_SCREWLESS_MESH_VALIDATION_PASSED"
    )
    return 0
