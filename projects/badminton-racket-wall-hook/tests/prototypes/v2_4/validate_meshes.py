#!/usr/bin/env python3
"""Validate v2.4 3MF meshes and protected v2.2/v2.3 assets with the stdlib."""
...
EXPECTED_COMPONENTS = {
    "v2_4_full_body_sections.3mf": 11,
    "v2_4_full_rack_collars.3mf": 10,
    "v2_4_full_rack_cradle.3mf": 1,
    "v2_4_skadis_back_two_hook.3mf": 1,
    "v2_4_multiboard_back_two_mount.3mf": 1,
    "v2_4_skadis_fit_coupon_two_hook.3mf": 1,
    "v2_4_multiboard_fit_coupon_two_mount_50mm.3mf": 1,
}
PRINT_BED_XY = (256.0, 256.0)
PROJECT_ROOT = Path(__file__).resolve().parents[3]
PROTECTED_MANIFEST = Path(__file__).with_name(
    "protected_v2_2_v2_3.sha256"
)
...
def verify_protected_assets() -> list[str]:
    ...
    if not PROTECTED_MANIFEST.is_file():
        return [f"missing protected-asset manifest {PROTECTED_MANIFEST}"]

    checked = 0
    for line_number, raw_line in enumerate(
        PROTECTED_MANIFEST.read_text(encoding="utf-8").splitlines(),
        start=1,
    ):
        line = raw_line.strip()
        ...
        try:
            ...
            failures.append(
                f"{PROTECTED_MANIFEST.name}:{line_number}: malformed entry"
            )
            continue

        path = PROJECT_ROOT / relative_path.strip()
        if not path.is_file():
            failures.append(f"protected asset is missing: {relative_path}")
            continue
    ...
    if checked == 0:
        failures.append("protected-asset manifest contains no files")
    else:
        print(f"Protected v2.2/v2.3 assets: {checked} checked")
    return failures
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        nargs="?",
        type=Path,
        default=Path("files/prototypes/v2.4"),
    )
    ...
    for filename, expected_components in EXPECTED_COMPONENTS.items():
        ...
        if not bed_ok:
            failures.append(
                f"{filename}: XY bounds {result['size'][0]:.3f} x "
                f"{result['size'][1]:.3f} exceed "
                f"{PRINT_BED_XY[0]:.0f} x {PRINT_BED_XY[1]:.0f} mm"
            )
    ...
    print("\nV2_4_PRINTABLE_FULL_RACK_MESH_VALIDATION_PASSED")
    return 0
