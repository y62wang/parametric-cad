#!/usr/bin/env python3
"""Validate assembled v2.7 clearances using generated OpenSCAD Booleans."""
...
SKADIS_HOOK_WIDTHS = (4.0, 4.2, 4.4)
SKADIS_BOARD_THICKNESSES = (2.6, 3.0, 4.0, 5.0)
...
def run_boolean(
    ...
    coupon_index: int | None = None,
    defines: tuple[tuple[str, float], ...] = (),
) -> MeshEvidence:
    suffix_parts = []
    if sample_z is not None:
        suffix_parts.append(
            f"{sample_z:.2f}".replace(".", "_")
        )
    if coupon_index is not None:
        suffix_parts.append(str(coupon_index))
    for name, value in defines:
        suffix_parts.append(
            f"{name}_{value:.3f}".replace(".", "_")
        )
    suffix = (
    ...
    if coupon_index is not None:
        command.extend(
            ["-D", f"coupon_index={coupon_index}"]
        )
    for name, value in defines:
        command.extend(["-D", f"{name}={value:.6f}"])
    ...
    if completed.returncode != 0:
        raise RuntimeError(
            f"OpenSCAD exited {completed.returncode} for {case}:\n"
            f"{completed.stdout}"
        )
    raise RuntimeError(
...
def require_positive_volume(
    label: str,
    evidence: MeshEvidence,
    failures: list[str],
) -> None:
    print(f"{label}: {format_evidence(evidence)}")
    if not evidence.has_positive_volume:
        failures.append(
            f"{label} did not preserve positive overlap: "
            f"{format_evidence(evidence)}"
        )
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(
        prefix="badminton_v2_7_interference_"
        ...
        connector_proxy = run_boolean(
            openscad,
            output_directory,
            "multiboard_connector_vs_proxy",
        )
        require_zero_positive_volume(
            "Multiboard seated catch vs proxy board",
            connector_proxy,
            failures,
        )

        pads_proxy = run_boolean(
            openscad,
            output_directory,
            "multiboard_pads_vs_proxy",
        )
        require_zero_positive_volume(
            "Multiboard bearing lands vs proxy board",
            pads_proxy,
            failures,
        )

        tip_stop = run_boolean(
            openscad,
            output_directory,
            "multiboard_tip_vs_rear_stop",
        )
        require_zero_positive_volume(
            "Multiboard tip vs rear-clearance stop",
            tip_stop,
            failures,
        )

        plug_pad = run_boolean(
            openscad,
            output_directory,
            "multiboard_plug_vs_bearing_pad",
        )
        require_positive_volume(
            "Multiboard plug vs bearing-land root",
            plug_pad,
            failures,
        )

        for coupon_index, board_t in enumerate(
            (5.0, 6.0, 7.0)
        ):
            depth_connector_proxy = run_boolean(
                openscad,
                output_directory,
            ...
            require_zero_positive_volume(
                "Multiboard depth coupon "
                f"{board_t:.0f} mm catch vs proxy",
                depth_connector_proxy,
                failures,
            )

            depth_pads_proxy = run_boolean(
                openscad,
                output_directory,
            ...
            require_zero_positive_volume(
                "Multiboard depth coupon "
                f"{board_t:.0f} mm pads vs proxy",
                depth_pads_proxy,
                failures,
            )

            depth_tip_stop = run_boolean(
                openscad,
                output_directory,
            ...
            require_zero_positive_volume(
                "Multiboard depth coupon "
                f"{board_t:.0f} mm tip vs rear stop",
                depth_tip_stop,
                failures,
            )

            depth_shoulder = run_boolean(
                openscad,
                output_directory,
            ...
            require_positive_volume(
                "Multiboard depth coupon "
                f"{board_t:.0f} mm shoulder behind board",
                depth_shoulder,
                failures,
            )

        skadis_pads_proxy = run_boolean(
            openscad,
            output_directory,
            "skadis_lower_pads_vs_seated_proxy",
        )
        require_zero_positive_volume(
            "SKADIS lower pads vs coherent seated board",
            skadis_pads_proxy,
            failures,
        )

        for hook_width in SKADIS_HOOK_WIDTHS:
            for board_thickness in SKADIS_BOARD_THICKNESSES:
                selector_overrides = (
                    (
                ...
                selector_label = (
                    f"SKADIS hook {hook_width:.1f} mm, "
                    f"board {board_thickness:.1f} mm"
                )

                skadis_hooks_proxy = run_boolean(
                    openscad,
                    output_directory,
                    "skadis_hooks_vs_seated_proxy",
                    defines=selector_overrides,
                )
                require_zero_positive_volume(
                    f"{selector_label} vs seated rounded slots",
                    skadis_hooks_proxy,
                    failures,
                )

                for side in ("left", "right"):
                    skadis_capture = run_boolean(
                        openscad,
                        output_directory,
                        f"skadis_{side}_capture_behind_board",
                        defines=selector_overrides,
                    )
                    require_positive_volume(
                        f"{selector_label}, {side} retention",
                        skadis_capture,
                        failures,
                    )

        skadis_fillet = run_boolean(
            openscad,
            output_directory,
            "skadis_root_fillet_slot_intrusion",
        )
        require_zero_positive_volume(
            "SKADIS root fillet behind board-contact plane",
            skadis_fillet,
            failures,
        )
    ...
    print(
        "\nV2_7_ASSEMBLED_INTERFERENCE_VALIDATION_PASSED"
    )
    return 0
